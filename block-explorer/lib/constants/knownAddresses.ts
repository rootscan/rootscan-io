...
export const contractAddressToNativeId = (contractAddress: Address): string | null => {
  ...
  return parseInt(contractAddress?.substring(10, contractAddress?.length - 24), 16).toString(16);
};

export const getAddressName = (address: Address, useShortenedAddress = false) => {
  const nativeId = contractAddressToNativeId(address);
  const shortenedAddress = nativeId
    ? address?.substring(0, 4) + `..${nativeId}..` + address?.substring(address?.length - 4, address?.length)
