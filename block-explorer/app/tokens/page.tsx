      ...
      <Table>
        <TableCaption>
          <TableNavigation pagination={getPaginationData(data)}>
            {!data.skipFullCount && <p>Showing {numberFormatter.format(data.totalDocs)} tokens</p>}
          </TableNavigation>
        </TableCaption>
        <TableHeader>
          <TableRow>
            ...
            <TableRow key={token.contractAddress}>
              <TableCell className="max-w-xs truncate">
                <div className="flex items-center gap-2">
                  <TokenLogo contractAddress={token.contractAddress} width={32} height={32} />
              ...
              <TableCell>
                <Badge size="sm">{token?.type}</Badge>
              </TableCell>
              <TableCell>{token?.collectionId ? token?.collectionId : token?.assetId ? token?.assetId : '-'}</TableCell>
              <TableCell>
